...
export default function CreatePostForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(
        `[CreatePostForm] Submitting post with userId: ${userId}, content length: ${trimmed.length}`
      );
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="p-4">
        <div className="relative cursor-text" onClick={() => !isExpanded && setIsExpanded(true)}>
          {isExpanded ? (
            <PostEditor
              value={content}
            ...
            >
              ...
              {showMediaUploader && (
                <div className="p-3 border border-gray-200 rounded-md bg-gray-50">
                  <MediaUploader onUploadComplete={handleMediaUpload} type="post" />
                </div>
              )}
