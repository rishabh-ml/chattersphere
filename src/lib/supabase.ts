...
if (USE_MOCK) {
  console.warn(
    "Using mock Supabase implementation. Set localStorage.USE_SUPABASE_MOCK = false to use real Supabase."
  );
}
...
export async function uploadFileFromUrl(
  ...
  try {
    ...
    // Upload the file
    const { data, error } = await supabaseAdmin.storage.from(bucket).upload(fileName, blob, {
      cacheControl: "3600",
      upsert: false,
    });
  ...
  try {
    ...
    return data.map((file: any) => ({
      ...
      url: supabaseAdmin.storage
        .from(bucket)
        .getPublicUrl(path ? `${path}/${file.name}` : file.name).data.publicUrl,
    }));
  } catch (error) {
