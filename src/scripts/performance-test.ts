...
async function testDatabaseQueries() {
  ...
  const queryComparison = await compareDatabaseQueries(findQuery, aggregateQuery, DB_TEST_ITERATIONS);
  ...
  console.log(`  Difference: ${queryComparison.difference.averageTime.toFixed(2)}ms (${queryComparison.difference.percentage.toFixed(2)}%)`);
  ...
  const commentsComparison = await compareDatabaseQueries(commentsQuery, commentsAggregateQuery, DB_TEST_ITERATIONS);
  
  console.log(`  Find query average time: ${commentsComparison.queryA.averageTime.toFixed(2)}ms`);
  console.log(`  Aggregate query average time: ${commentsComparison.queryB.averageTime.toFixed(2)}ms`);
  console.log(`  Difference: ${commentsComparison.difference.averageTime.toFixed(2)}ms (${commentsComparison.difference.percentage.toFixed(2)}%)`);
  ...
  const membersComparison = await compareDatabaseQueries(membersQuery, membersAggregateQuery, DB_TEST_ITERATIONS);
  
  console.log(`  Find query average time: ${membersComparison.queryA.averageTime.toFixed(2)}ms`);
  console.log(`  Aggregate query average time: ${membersComparison.queryB.averageTime.toFixed(2)}ms`);
  console.log(`  Difference: ${membersComparison.difference.averageTime.toFixed(2)}ms (${membersComparison.difference.percentage.toFixed(2)}%)`);
