...
async function createPostIndexes() {
  try {
    ...
    await Post.collection.createIndex({ content: 'text', title: 'text' }, { name: 'post_text_index' });
...
async function createUserIndexes() {
  try {
    ...
    // Text index for search
    await User.collection.createIndex({ username: 'text', name: 'text', bio: 'text' }, { name: 'user_text_index' });
...
async function createCommunityIndexes() {
  try {
    ...
    // Text index for search
    await Community.collection.createIndex({ name: 'text', description: 'text' }, { name: 'community_text_index' });
