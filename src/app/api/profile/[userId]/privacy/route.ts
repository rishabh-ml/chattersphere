...
export async function GET(
  ...
  try {
    ...
    if (user.clerkId !== clerkUserId) {
      return NextResponse.json({ error: "Unauthorized: You can only view your own privacy settings" }, { status: 403 });
    }
...
export async function PUT(
  ...
  try {
    ...
    // Check if the requesting user is the profile owner
    if (user.clerkId !== clerkUserId) {
      return NextResponse.json({ error: "Unauthorized: You can only update your own privacy settings" }, { status: 403 });
    }
