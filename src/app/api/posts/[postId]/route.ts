  ...
  try {
    ...
    if (post.community) {
      ...
      if (community && community.isPrivate) {
        ...
        if (!userId) {
          console.log(`[POSTS GET] Unauthorized access attempt to post ${sanitizedPostId} in private community: ${post.community._id}`);
        ...
        if (!isMember) {
          console.log(`[POSTS GET] Forbidden access attempt by user ${user._id} to post ${sanitizedPostId} in private community: ${post.community._id}`);
          return NextResponse.json({
            error: "You are not authorized to view posts in this private community"
          }, { status: 403 });
        }

        console.log(`[POSTS GET] Authorized access to post ${sanitizedPostId} in private community ${post.community._id} by member ${user._id}`);
      }
    }
