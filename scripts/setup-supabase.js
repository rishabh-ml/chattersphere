...
console.log(
  "Service Role Key:",
  serviceRoleKey ? `${serviceRoleKey.substring(0, 10)}...` : "Not set"
);
  ...
  try {
    ...
    if (testError) {
      if (testError.message && testError.message.includes("ENOTFOUND")) {
        ...
        testError.message &&
        testError.message.includes('relation "_test" does not exist')
      ) {
    ...
    const { data: avatarsBucket, error: avatarsError } =
      await supabaseAdmin.storage.getBucket("avatars");
    ...
    const { data: mediaBucket, error: mediaError } = await supabaseAdmin.storage.getBucket("media");
    ...
    const { error: avatarsPolicyError } = await supabaseAdmin.storage
      .from("avatars")
    ...
    const { error: mediaPolicyError } = await supabaseAdmin.storage
      .from("media")
      .createSignedUrl("test.txt", 60);
